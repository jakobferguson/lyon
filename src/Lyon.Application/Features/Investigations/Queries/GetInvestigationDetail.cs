...
public record GetInvestigationDetailQuery(Guid IncidentId) : IRequest<InvestigationDetailResponse?>;

public class GetInvestigationDetailQueryHandler : IRequestHandler<GetInvestigationDetailQuery, InvestigationDetailResponse?>
{
    private readonly IApplicationDbContext _db;

    public GetInvestigationDetailQueryHandler(IApplicationDbContext db) => _db = db;

    public async Task<InvestigationDetailResponse?> Handle(GetInvestigationDetailQuery query, CancellationToken ct)
    {
        var investigation = await _db.Investigations
            ...
            .Where(i => i.IncidentId == query.IncidentId && !i.IsDeleted)
            .OrderByDescending(i => i.InvestigationNumber)
